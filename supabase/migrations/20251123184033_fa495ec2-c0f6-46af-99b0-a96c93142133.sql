-- Corriger compute_chantier_metrics pour utiliser montant_ht au lieu de montant_ttc
...
DECLARE
  ...
  _cout_jour_equipe_aff NUMERIC := 0;
  _cout_jour_equipe_eq NUMERIC := 0;
  ...
  _marge_finale NUMERIC := 0;
  _marge_finale_pct NUMERIC := 0;
  _statut TEXT := 'VERT';
  _etat_chantier TEXT;
BEGIN
  -- Budget depuis devis actif (MONTANT HT) ou chantier + récupération de l'état
  SELECT 
    COALESCE(
      (SELECT montant_ht FROM devis WHERE chantier_id = p_chantier AND actif = true LIMIT 1),
      c.budget_ht,
      0
    ),
    c.date_debut_prevue,
    ...
    c.etat_chantier
  INTO _budget_ht, _date_debut, _duree_estimee, _etat_chantier
  FROM chantiers c
  WHERE c.id = p_chantier;

  -- Coût journalier équipe depuis les affectations détaillées
  SELECT COALESCE(SUM(
  ...
  INTO _cout_jour_equipe_aff
  FROM affectations_chantiers a
  JOIN membres_equipe m ON m.id = a.membre_equipe_id
  WHERE a.chantier_id = p_chantier;

  -- Coût journalier équipe depuis equipe_chantier
  SELECT COALESCE(SUM(
    m.taux_horaire * 8 *
    (1 + COALESCE(m.charges_salariales_pct, m.charges_salariales, 0)/100 +
         COALESCE(m.charges_patronales_pct, m.charges_patronales, 0)/100)
  ), 0)
  INTO _cout_jour_equipe_eq
  FROM equipe_chantier ec
  JOIN membres_equipe m ON m.id = ec.membre_id
  WHERE ec.chantier_id = p_chantier;

  -- Coût journalier total de l'équipe
  _cout_jour_equipe := _cout_jour_equipe_aff + _cout_jour_equipe_eq;

  -- Coûts fixes (frais + factures)
  ...
  -- Si le chantier est terminé ou annulé, jours restants = NULL
  IF _etat_chantier IN ('termine', 'annule') THEN
    _jour_critique := NULL;
    _jours_restants_rentables := NULL;
  ELSIF _cout_jour_equipe > 0 THEN
  ...
  -- Calcul de la marge finale (projection à la fin du chantier)
  _marge_finale := _budget_ht - _couts_fixes - (_cout_jour_equipe * _duree_estimee);
  
  IF _budget_ht > 0 THEN
    _marge_finale_pct := ROUND((_marge_finale / _budget_ht) * 100, 2);
  ELSE
    _marge_finale_pct := 0;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'marge_finale', ROUND(_marge_finale, 2),
    'marge_finale_pct', _marge_finale_pct,
    'statut_rentabilite', _statut
  );
END;
$function$;

-- Recalculer toutes les métriques avec les montants HT corrects
INSERT INTO chantier_metrics_realtime (chantier_id, metrics, updated_at)
SELECT id, compute_chantier_metrics(id), now()
FROM chantiers
